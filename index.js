...
function getArtistByIndex(array, index) {
  let artistIndex = `the artist at index ${index} is ${array[index].name}`;
  return artistIndex; 
}
...
function lotsOfArt(array) {
  let filterArt = array.filter(object => object.paintings > 100);
  let art = [];
  for (let i = 0; i < filterArt.length; i++) {
    art.push(filterArt[i].name);
    }
  return art;
}
